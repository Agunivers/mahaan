    ...
    if chat.type != chat.PRIVATE:
        if len(args) >= 1:
            ...
            if (var == "no"):
                ...
                update.effective_message.reply_text("اتصالات المعوقين إلى هذه الدردشة للمستخدمين")
            elif(var == "yes"):
                sql.set_allow_connect_to_chat(chat.id, True)
                update.effective_message.reply_text("جهات الاتصالات ممكنة إلى هذه الدردشة للمستخدمين")
            else:
                update.effective_message.reply_text("رجاء ادخل on/yes/off/no في المجموعة!")
        else:
            update.effective_message.reply_text("رجاء ادخل on/yes/off/no في المجموعة!")
    else:
        update.effective_message.reply_text("رجاء ادخل on/yes/off/no في المجموعة!")
    ...
    if update.effective_chat.type == 'private':
        if len(args) >= 1:
            try:
                ...
                update.effective_message.reply_text("معرف الدردشة غير صالح المقدمة!")
            if (bot.get_chat_member(connect_chat, update.effective_message.from_user.id).status in ('administrator', 'creator') or 
                ...
                if connection_status:
                    ...
                    update.effective_message.reply_text("متصل بنجاح ب * {} *".format(chat_name), parse_mode=ParseMode.MARKDOWN)
                ...
                update.effective_message.reply_text("اتصالات إلى هذه الدردشة غير مسموح بها!")
        else:
            update.effective_message.reply_text("إدخال محادثة المدخلات للاتصال!")
            history = sql.get_history(user.id)
            print(history.user_id, history.chat_id1, history.chat_id2, history.chat_id3, history.updated)
        ...
        if (bot.get_chat_member(conn_id, user_id).status in ('administrator', 'creator') or 
            ...
            if need_admin == True:
                if bot.get_chat_member(conn_id, update.effective_message.from_user.id).status in ('administrator', 'creator') or user_id in SUDO_USERS:
                    ...
                    update.effective_message.reply_text("تحتاج إلى أن تكون مسؤول في مجموعة متصلة!")
                    exit(1)
            else:
                return conn_id
        else:
            update.effective_message.reply_text("مجموعة تغيير الحقوق أو أنت غير مسؤول بعد الآن.\nسيتم قطع اتصالك")
...
تتوفر الإجراءات مع المجموعات المتصلة:
 ...
 - /connect <chatid>: الاتصال بالدردشة عن بعد
 - /disconnect: قطع الاتصال من الدردشة
 - /allowconnect on/yes/off/no: السماح بتوصيل المستخدمين بمجموعة
"""
