    ...
    if can_delete(chat, bot.id):
        if len(args) >= 1:
            if args[0] in LOCK_TYPES:
                ...
                message.reply_text("مقفل {} رسائل لجميع غير مدرجين!".format(args[0]))
                ...
                message.reply_text("ماذا تحاول قفل ...؟ حاول / locktyes لقائمة القفل")
    ...
    if is_user_admin(chat, message.from_user.id):
        if len(args) >= 1:
            if args[0] in LOCK_TYPES:
                ...
                message.reply_text("غير مقفلة {} للجميع!".format(args[0]))
                return "<b>{}:</b>" \
                       "\n#UNLOCK" \
                       "\n<b>Admin:</b> {}" \
                ...
                message.reply_text("ما الذي تحاول فتح ...؟ حاول / locktypes لقائمة القفل")

        else:
            bot.sendMessage(chat.id, "ما الذي تحاول فتحة...؟ هذا للأدمن فقط")
                ...
                for new_mem in new_members:
                    if new_mem.is_bot:
                        if not is_bot_admin(chat, bot.id):
                            message.reply_text("أرى بوت، وقيل لي أن أوقفهم عن الانضمام ... "
                                               "لكنني لست المسؤول!")
                            return

                        chat.kick_member(new_mem.id)
                        message.reply_text("يسمح للمحادثات فقط بإضافة روبوتات إلى هذه الدردشة! الحصول على outta هنا")
            else:
                try:
                    message.delete()
...
__help__ = """
 - /locktypes: قائمة من قفل الكتابة المحتملة

*للمشرف فقط*
 - /lock <type>: قفل العناصر من نوع معين (غير متوفر في القطاع الخاص)
 - /unlock <type>: فتح عناصر من نوع معين (غير متوفر بالخصوصية)
 - /locks: قائمة الأقفال الحالية في هذه الدردشة.

يمكن استخدام الأقفال لتقييد مستخدمي المجموعة.
على سبيل المثال: ستحذف قفل عناوين URL من تلقائيا جميع الرسائل مع عناوين URL التي لم تكن مبالغة، ستتحذف ملصقات القفل جميع \ الملصقات، وما إلى ذلك. سيقوم قفل الروبوتات بإدخال الإيدح من إضافة الروبوتات إلى الدردشة.
"""
