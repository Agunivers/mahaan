...
def list_urls(bot, update):
    ...
    if len(final_content) == 0:
        bot.send_message(chat_id=tg_chat_id, text="هذه الدردشة غير مشترك في أي روابط")
    elif len(final_content) <= constants.MAX_MESSAGE_LENGTH:
        bot.send_message(chat_id=tg_chat_id, text="تم الاشتراك في هذه الدردشة على الروابط التالية:\n" + final_content)
    else:
        bot.send_message(chat_id=tg_chat_id, parse_mode=ParseMode.HTML,
                         text="<b>تحذير:</b> الرسالة طويلة جدا ليتم إرسالها")
...
__help__ = """
 - /addrss <link>: أضف رابط RSS إلى الاشتراكات.
 - /removerss <link>: يزيل رابط RSS من الاشتراكات.
 - /rss <link>: يظهر بيانات الارتباط والدخول الأخير، لأغراض الاختبار.
 - /listrss: يوضح قائمة RSS تغذية أن الدردشة مشترك حاليا في.

ملاحظة: في مجموعات، يمكن للمديرين فقط إضافة / إزالة روابط RSS إلى اشتراك المجموعة
"""
