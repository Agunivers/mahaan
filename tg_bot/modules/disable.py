...
if is_module_loaded(FILENAME):
    ...
    def disable(bot: Bot, update: Update, args: List[str]):
        ...
        if len(args) >= 1:
            ...
            if disable_cmd in set(DISABLE_CMDS + DISABLE_OTHER):
                ...
                update.effective_message.reply_text("تعطيل استخدام `{}`".format(disable_cmd),
                                                    parse_mode=ParseMode.MARKDOWN)
            else:
                update.effective_message.reply_text("لا يمكن تعطيل هذا الأمر")

        else:
            update.effective_message.reply_text("ماذا يجب أن تعطل؟")
    ...
    def enable(bot: Bot, update: Update, args: List[str]):
        ...
        if len(args) >= 1:
            ...
            if sql.enable_command(chat.id, enable_cmd):
                update.effective_message.reply_text("مكن استخدام `{}`".format(enable_cmd),
                                                    parse_mode=ParseMode.MARKDOWN)
            else:
                update.effective_message.reply_text("هل هذا حتى المعطلين؟")

        else:
            update.effective_message.reply_text("ماذا يجب أن تمكن؟")
    ...
    def list_cmds(bot: Bot, update: Update):
        if DISABLE_CMDS + DISABLE_OTHER:
            ...
            update.effective_message.reply_text("الأوامر التالية هي Toggleable: \n {}".format(result),
                                                parse_mode=ParseMode.MARKDOWN)
        else:
            update.effective_message.reply_text("لا يمكن تعطيل أوامر.")
    ...
    __help__ = """
 - /cmds: تحقق من الوضع الحالي لأوامر المعوقين

*Admin only:*
 - /enable <cmd name>: تمكين هذا الأمر
 - /disable <cmd name>: تعطيل هذا الأمر
 - /listcmds: قائمة جميع الأوامر الممكنة مسبقاً 
    """
