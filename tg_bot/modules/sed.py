...
def sed(bot: Bot, update: Update):
    ...
    if sed_result and update.effective_message.reply_to_message:
        ...
        if not repl:
            update.effective_message.reply_to_message.reply_text("أنت تحاول استبدالها ... "
                                                                 "لا شيء مع شيء ما؟")
            return
        ...
        try:
            ...
            if check and check.group(0).lower() == to_fix.lower():
                update.effective_message.reply_to_message.reply_text("يا الجميع، {} تحاول صنع"
                                                                     "أنا أقول أشياء لا أريدها"
                                                                     "ويقول!".format(update.effective_user.first_name))
                return
        ...
        if len(text) >= telegram.MAX_MESSAGE_LENGTH:
            update.effective_message.reply_text("وكانت نتيجة الأمر SED طويلا جدا بالنسبة \
                                                 telegram!")
        elif text:
